...
@Component({
    selector: 'jhi-message-month-date-picker',
    templateUrl: './month-date-picker.component.html',
    styleUrls: ['./month-date-picker.component.css'],
    providers: [
        {
            provide: NG_VALUE_ACCESSOR,
            useExisting: forwardRef(() => MonthDateMessagePickerComponent),
            multi: true
        }
    ]
})
export class MonthDateMessagePickerComponent implements ControlValueAccessor {
    data: Idata;
    dataTxt: string;
    separator: string;
    monthFirst: boolean;
    ...
    change(value: string) {
        value =
            this.separator === ' '
                ...
                : this.separator === '/'
                    ? value.replace(/\.|-| /, '/')
                    : this.separator === '-' ? value.replace(/\.| |\//, '-') : value.replace(/.| |\/ /, '-');
    ...
    formatData(data: Idata): string {
        ...
        return this.monthFirst ? monthTxt + this.separator + data.year : '' + data.year + this.separator + monthTxt;
    }
